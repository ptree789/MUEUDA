...
class CoCoOp(TrainerX):
    ...
    def build_model(self):
        ...
        self.visual_teacher = copy.deepcopy(self.model.image_encoder)
        alpha=0.9
        pseudo_label_weight='none'
        self.ema_teacher = EMATeacher(self.visual_teacher, alpha=alpha, pseudo_label_weight=pseudo_label_weight)
        ...
        # 同步权重到EMA教师模型
        #self.ema_teacher.update_weights(self.model.image_encoder)
    ...
    def forward_backward(self, batch):
        ...
        # 仅在最后一个 batch 时更新 EMA 教师模型权重
        #print("self.num_batch", self.num_batches)
        #print("self.batch_idx", self.batch_idx)
        if (self.batch_idx + 1) == self.num_batches:
            self.ema_teacher.update_weights(self.model.module.image_encoder, self.epoch)
